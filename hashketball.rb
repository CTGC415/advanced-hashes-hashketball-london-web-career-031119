...
def team_colors(team)
  game_hash.collect do |location, team_data|
    team_data[:team_name] == team ? (return team_data[:colors]) : false
  end
end
...
def player_numbers(team)
  game_hash.each do |location, team_data|
    if team_data[:team_name].to_s == team
      return team_data[:players].collect { |player, data| data[:number] }
    end
  end
end
...
def player_stats(name)
  ...
  game_hash.each do |location, team_data|
    team_data[:players].each do |player, data|
      if name == player.to_s
        return data.each { |stat, data| stats.merge(stat=>data) }
      end
    end
  end
end
...
def big_shoe_rebounds
  ...
  game_hash.each do |location, team_data|
    team_data[:players].each do |player, data|
      stats_list << player_stats(player.to_s)
    end
  end
